...
class TerminalWidget(QWidget):
    ...
    def run_beam_file(self, file_path: str, beam_executable: str = None):
        """Run a BEAM file using the BEAM interpreter"""
        ...
        if beam_executable is None:
            beam_executable = self._find_beam_interpreter()

        if not beam_executable:
            self.clear()
            self.write_line("[ERROR] BEAM interpreter not found!", self.theme.error if self.theme else None)
            self.write_line("")
            self.write_line("Please ensure the 'beam' executable is:")
        ...
        self.write_line(f"[>] Interpreter: {beam_executable}", self.theme.info if self.theme else None)
        ...
        # Strip LD_LIBRARY_PATH entirely before launching beam.
        # PyInstaller, Electron (VS Code), and similar app wrappers all prepend
        # their own bundled libs to LD_LIBRARY_PATH. Beam only needs system
        # SDL2 which ldconfig finds without any LD_LIBRARY_PATH at all.
        env = QProcessEnvironment.systemEnvironment()
        env.remove("LD_LIBRARY_PATH")
        self.process.setProcessEnvironment(env)

        self.process.start(beam_executable, args)
    ...
    def run_external_file(self, file_path: str):
        """Run a BEAM file in an external terminal"""
        beam_executable = self._find_beam_interpreter()
        if not beam_executable:
             self.write_line("[ERROR] BEAM interpreter not found!", self.theme.error if self.theme else None)
             return
        ...
        # e.g. beam /path/to/file.bas
        
        if sys.platform == "win32":
            # Windows: cmd /k keeps window open
            beam_executable_abs = str(Path(beam_executable).resolve())
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.bat', delete=False) as f:
                ...
                f.write(f'"{beam_executable_abs}" "{file_path_abs}"\n')
                f.write('echo.\n')
                f.write('pause\n')
                temp_batch = f.name
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.command', delete=False) as f:
                f.write('#!/bin/bash\n')
                f.write(f'"{beam_executable}" "{file_path}"\n')
            ...
            # Linux
            wrapper_cmd = f'"{beam_executable}" "{file_path}"; echo; read -p "Press Enter to close..."'
            run_args = ["bash", "-c", wrapper_cmd]
            final_cmd = list(cmd_parts)
            final_cmd.extend(run_args)

            # Strip LD_LIBRARY_PATH so the external terminal and beam don't
            # inherit bundled libs from PyInstaller / Electron / etc.
            ext_env = os.environ.copy()
            ext_env.pop("LD_LIBRARY_PATH", None)

            try:
                subprocess.Popen(final_cmd, env=ext_env)
    ...
    def _find_beam_interpreter(self) -> str:
        """Find the BEAM interpreter using multiple strategies"""
        import shutil
        import sys

        # Strategy 1: Check settings for configured path
        if self.settings and self.settings.settings.beam_interpreter_path:
            path = Path(self.settings.settings.beam_interpreter_path)
            if path.exists() and path.is_file():
                return str(path)

        beam_name = "beam.exe" if sys.platform == "win32" else "beam"

        # Strategy 2: Check bundled/local location before PATH.
        # PATH is checked last because other distros may have an unrelated
        # program named "beam" (e.g. a system package) that would be found
        # first and produce blank/broken windows.
        if getattr(sys, 'frozen', False):
            # PyInstaller 6.x: _MEIPASS points to _internal/ where binaries are bundled
            if hasattr(sys, '_MEIPASS'):
                meipass_beam = Path(sys._MEIPASS) / beam_name
                if meipass_beam.exists():
                    return str(meipass_beam)
            exe_dir = Path(sys.executable).parent
        else:
            # Running from source — beam_ide/app/terminal.py → go up to Beam/
            exe_dir = Path(__file__).parent.parent.parent

        beam_exe = exe_dir / beam_name
        if beam_exe.exists():
            return str(beam_exe)

        # Strategy 3: Check parent directory
        if sys.platform == "win32":
            parent_beam = exe_dir.parent / "beam.exe"
            if parent_beam.exists():
                return str(parent_beam)

        parent_beam = exe_dir.parent / "beam"
        if parent_beam.exists():
            return str(parent_beam)

        # Strategy 4: Fall back to PATH (last resort — may find unrelated binary)
        beam_in_path = shutil.which("beam")
        if beam_in_path:
            return beam_in_path
